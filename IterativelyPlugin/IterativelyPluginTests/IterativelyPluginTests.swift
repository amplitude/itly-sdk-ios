...
class IterativelyPluginTests: XCTestCase {
    ...
    override func setUp() {
        ...
        do {
            self.plugin = try IterativelyPlugin("api-key", options: IterativelyOptions())
        } catch {
...
//     func test_dataplaneJson_withBranchAndVersion_hasBranchAndVersionInJson() throws {
//         let options = IterativelyOptions(branch: "main", version: "1.2.3")
//         let plugin = try IterativelyPlugin("api-key", options: options)
//
//         XCTAssertEqual(plugin.url(), url)
//         XCTAssertNotEqual(plugin.url(), options.url)
